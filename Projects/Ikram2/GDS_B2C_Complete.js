...
var B2C_CONFIG = {
  ...
  // قدوم - الرحلة 1 (AG-AM) = أعمدة 33-39
  DEP1: { fn: 33, dd: 34, dt: 35, fr: 36, to: 37, ad: 38, at: 39 },
  // قدوم - الرحلة 2 (AN-AT) = أعمدة 40-46
...
function setupPDFExtraction_() {
  ...
  // المباشر يُكتب في DEP2 — نفحصه لاكتشاف المعبأ
 ...
 *   1. مباشرة مكررة (DEP1 = DEP2 نفس الرحلة) → مسح DEP1
 *   2. مباشرة في DEP1 بدون DEP2 → نقل DEP1 → DEP2
 *   3. عودة مكررة (RET1 = RET2) → مسح RET2
 */
function fixClassification() {
  var ss = SpreadsheetApp.openById(B2C_CONFIG.SPREADSHEET_ID);
  var sheet = ss.getSheetByName(B2C_CONFIG.SHEET_NAME);
  if (!sheet) { safeUI_().alert('شيت B2C غير موجود'); return; }
  var ui = safeUI_();
  var lastRow = sheet.getLastRow();
  ...
  var fixedDup = 0;      // قدوم مكرر → مسح DEP1
...
function sendDailyReport() {
  ...
  for (var i = 0; i < numRows; i++) {
    var passport = cleanVal_(allData[i][B2C_CONFIG.COL_PASSPORT - 1]);
    if (!passport) continue;
    total++;
    
    var dep2Fn = cleanVal_(allData[i][B2C_CONFIG.DEP2.fn - 1]);  // AN
    var arrDate = cleanDate_(allData[i][B2C_CONFIG.DEP2.ad - 1]); // AS
    var arrTime = cleanVal_(allData[i][B2C_CONFIG.DEP2.at - 1]);  // AT

    var ret1Fn = cleanVal_(allData[i][B2C_CONFIG.RET1.fn - 1]);   // AU
    var retDate = cleanDate_(allData[i][B2C_CONFIG.RET1.dd - 1]);  // AV
    var retTime = cleanVal_(allData[i][B2C_CONFIG.RET1.dt - 1]);   // AW
    
    var pnr = cleanVal_(allData[i][B2C_CONFIG.COL_PNR - 1]);      // BI
    ...
    if (dep2Fn && (!arrDate || !arrTime)) {
      missingArrival.push({
        name: name,
        flight: dep2Fn,
        date: cleanDate_(allData[i][B2C_CONFIG.DEP2.dd - 1]),
        arrDate: arrDate || '❌',
        arrTime: arrTime || '❌'
      });
    }
